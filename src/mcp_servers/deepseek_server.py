...
Аутентификация (в порядке приоритета):
  1. DEEPSEEK_AUTH_TOKEN — Bearer-токен из Local Storage (userToken), если задан.
  2. DEEPSEEK_SMIDV2     — используется как Bearer-токен (обычно совпадает с userToken).

Cookies ds_session_id, smidV2, .thumbcache_* уходят в заголовке Cookie.
"""
import asyncio
import json
import logging
...
COOKIE_PREF = os.getenv("DEEPSEEK_COOKIE_PREFERENCE", "").strip()
...
def _headers() -> dict[str, str]:
    ...
    if COOKIE_PREF:
        cookies.append(f"ds_cookie_preference={COOKIE_PREF}")
    if cookies:
...
async def _create_session(client: httpx.AsyncClient) -> str:
    ...
    if r.status_code >= 400:
        raise RuntimeError(
            f"DeepSeek HTTP {r.status_code} при создании сессии: "
            f"{r.text[:500]}"
        )
    data = r.json()
    ...
    logger.info("DeepSeek session created: %s", sid)
    return sid


def _extract_text(obj: Any) -> str:
    """Достаёт текст из SSE-чанка DeepSeek (формат меняется между версиями)."""
...
async def list_tools() -> list[Tool]:
    return [
        Tool(
            name="deepseek_chat",
            description=(
                "Быстрый запрос к DeepSeek (chat-режим). "
                "Подходит для классификации, суммаризации, генерации JSON."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "prompt": {"type": "string", "description": "Запрос пользователя"},
                    "system": {"type": "string", "description": "System prompt"},
                    "thinking": {"type": "boolean", "default": False,
                                 "description": "Включить режим рассуждений"},
                },
                "required": ["prompt"],
            },
        ),
        Tool(
            ...
            description=(
                "Запрос к DeepSeek с включённым режимом рассуждений (R1/thinking). "
                "Медленнее, но точнее на сложных задачах."
            ),
            inputSchema={
                "type": "object",
                "properties": {
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    if not (AUTH_TOKEN or SMIDV2 or DS_SESSION_ID):
        return [TextContent(
            type="text",
            text="DeepSeek не настроен: задайте DEEPSEEK_SMIDV2 или DEEPSEEK_AUTH_TOKEN."
        )]
    ...
    try:
        ...
        if not answer:
            return [TextContent(type="text", text="(пустой ответ от DeepSeek)")]
        return [TextContent(type="text", text=answer)]
    except httpx.HTTPStatusError as e:
        return [TextContent(
            type="text",
            text=f"HTTP {e.response.status_code}: {e.response.text[:500]}"
        )]
